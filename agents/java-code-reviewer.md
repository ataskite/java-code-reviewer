...
**完整的「模式 × 维度覆盖矩阵」**定义在 `../references/review-framework.md` 第 275-293 行（`## 审查模式 × 维度覆盖矩阵` 章节），请读取该章节确定各维度的启用粒度。以下为快速参考：
...
- **规则 1 — 模式矩阵过滤**：仅评估当前审查模式启用的维度（见 `../references/review-framework.md` 覆盖矩阵）
...
**各维度的详细审查标准和模式 × 维度覆盖矩阵**均定义在 `../references/review-framework.md` 中，请在审查前读取该文件获取完整框架。

---

## 审查输出格式

**完整的报告输出格式规范**定义在 `../references/report-format.md` 中，包括：审查配置快照、审查范围说明、执行摘要、各级别问题格式（P0/P1/P2/P3/待确认）、修复优先级、总结等。
