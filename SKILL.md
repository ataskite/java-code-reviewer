...
- **禁止使用 `AskUserQuestion` 工具**：交互式确认的所有步骤，绝对不允许调用 AskUserQuestion 工具
- **禁止使用任何结构化用户输入收集工具**：所有选项必须以纯文本形式直接输出到聊天中，等待用户自由输入回复
- **唯一允许的输出方式**：将选项文本作为普通聊天消息输出，输出后立即停止，等待用户在下一轮对话中回复
- **违反复核**：每次准备输出交互选项前，先自检——"我是否正准备在一次回复中输出多个步骤的选项？"如果是，立即停止，只保留第一个步骤

⚠️ **严格单步执行规则（必须遵守）**：

1. **一次只执行一个步骤**：每次只输出当前步骤的选项文本，然后**立即停止输出**，等待用户回复。**即使上一个步骤的回复确认和下一个步骤的选项，也必须在同一个回复中只出现一组选项**
...
B) standard — 标准审查...
```
这种合并展示是绝对禁止的，无论任何理由。每组选项必须独占一个回复，等用户回答后再出现下一组。
...
**⚠️ 此步骤结束后必须立即停止输出，等待用户回复后再进入下一步骤。不得在本步骤的回复中同时输出步骤3（审查范围）的选项。**
...
**⚠️ 此步骤结束后必须立即停止输出，等待用户回复后再进入下一步骤。不得在本步骤的回复中同时输出步骤4（审查模式）的选项。**
...
**⚠️ 此步骤结束后必须立即停止输出，等待用户回复后再进入下一步骤。不得在本步骤的回复中同时输出步骤5（飞书上传）的选项。**
...
1. **必须使用纯文本选项交互** ⚡：所有用户交互（分支选择、审查类型选择、模块选择、模式选择、飞书上传选项、执行确认）都必须通过**纯文本输出选项**、等待用户**自由文本输入**的方式完成。**禁止使用 AskUserQuestion 等结构化输入工具**
...
9. **禁止合并交互步骤** ⚡：每个回复中**只能出现一组交互选项**。输出当前步骤的选项文本后立即停止输出，等待用户回复。绝不允许在一次回复中展示多个步骤的选项（例如将「审查范围」和「审查模式」放在同一个回复中）
