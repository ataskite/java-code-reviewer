...
> **注意**：用户输入 A（确认执行）后进入代码审查阶段调用子agent；输入 B（取消）则终止流程。

**⚠️ 此步骤结束后必须立即停止输出，等待用户回复。用户确认后才能进入代码审查阶段。**
...
**执行脚本**：
```bash
bash scripts/phase5-prepare-incremental.sh "$PROJECT_DIR" {N}
```
...
完整的示例对话（5个场景：本地单模块、Git多模块、快速启动、参数校验失败、指定分支+飞书上传）详见 `{baseDir}/references/examples.md`。

核心要点回顾：
- 交互式模式：严格单步，每组选项独占一个回复
- 快速启动模式：预扫描 → 参数校验 → 直接启动子agent，无交互
- 子agent完成后：已上传飞书则显示精简摘要+链接，未上传则显示完整报告