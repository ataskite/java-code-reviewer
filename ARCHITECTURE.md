# Java Code Reviewer 技能架构说明

> **⚠️ 平台说明**：本技能为 **Claude Code 专用插件**，基于 Claude Code 的 Agent 机制和 Skill 规范开发。

## 📁 文件结构

```
java-code-reviewer/
├── SKILL.md                      # ⭐ 主技能定义文档
│   ├── 用途与触发场景
│   ├── 快速启动模式（参数规范 + 校验规则）
│   ├── 完整工作流程
│   │   ├── 模式判定（交互式 / 快速启动）
│   │   ├── 预扫描阶段（4脚本顺序执行）
│   │   ├── 交互式确认（严格单步 + 工具禁用）
│   │   └── 代码审查阶段：子Agent调用
│   ├── 重要规则（10条强制规则）
│   ├── 错误处理
│   └── 示例对话（引用 references/examples.md）
│
├── README.md                     # 项目说明文档
│   ├── 安装与验证
│   ├── 两种使用模式
│   ├── 审查模式与维度
│   ├── 脚本说明
│   └── 开发维护指南
│
├── .gitignore                    # Git 忽略规则
│
├── prompts/                      # 🤖 子Agent提示词目录
│   └── java-code-reviewer.md     # 子Agent提示词（引用式架构）
│       ├── 审查原则
│       ├── 审查模式定义（引用 review-framework.md）
│       ├── 外部参数注入规范
│       ├── Agent执行流程（6步）
│       ├── 审查框架引用 ⭐
│       ├── 核心审查目标
│       ├── 问题等级定义
│       ├── 评分标尺
│       ├── 输出格式引用 ⭐（引用 report-format.md）
│       └── 审查指南（强制规则）
│
├── scripts/                      # 🔧 可执行脚本目录（6个脚本）
│   ├── phase1-detect-project.sh  # 项目识别与准备
│   │   └── 输入：路径/Git URL → 输出：PROJECT_DIR + PROJECT_SOURCE
│   │
│   ├── phase2-detect-branches.sh # 分支探测
│   │   └── 输入：PROJECT_DIR → 输出：分支列表
│   │
│   ├── phase2-switch-branch.sh   # 分支切换
│   │   └── 输入：PROJECT_DIR + TARGET_BRANCH + PROJECT_SOURCE → 输出：切换结果
│   │
│   ├── phase3-project-scan.sh    # 项目预扫描
│   │   └── 输入：PROJECT_DIR → 输出：PROJECT_TYPE + MODULE列表
│   │
│   ├── phase4-detect-lark-plugin.sh # lark-cli 检测
│   │   └── 输入：无 → 输出：LARK_PLUGIN_INSTALLED
│   │
│   └── phase5-prepare-incremental.sh # 增量审查预处理
│       └── 输入：PROJECT_DIR + N → 输出：GIT_LOG + FILES + STATS
│
└── references/                   # 📚 参考文档目录（3个文件）
    ├── review-framework.md       # 15维度审查框架手册
    │   ├── 15个审查维度详细定义
    │   ├── 模式×维度覆盖矩阵
    │   └── 各模式的启用范围说明
    ├── report-format.md          # 审查报告输出格式规范
    └── examples.md               # 示例对话（5个场景）
```

---

## 🔄 运行框架

### 整体架构

```
┌─────────────────────────────────────────────────────────────────┐
│                         用户 (User)                             │
└──────────────────────────────┬──────────────────────────────────┘
                               │ 触发审查请求
                               ↓
┌─────────────────────────────────────────────────────────────────┐
│                    主 Agent (Master Agent)                      │
│                     SKILL.md 定义                               │
│                                                                 │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │  模式判定：检测输入是否包含 --mode 参数                   │  │
│  │  ├─ 包含 → 快速启动模式                                  │  │
│  │  └─ 不包含 → 交互式模式（默认）                          │  │
│  └──────────────────────────────────────────────────────────┘  │
│                                                                 │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │  预扫描阶段（自动执行，不与用户交互）                    │  │
│  │  → phase1-detect-project.sh → PROJECT_DIR + SOURCE       │  │
│  │  → phase2-detect-branches.sh → 分支列表                  │  │
│  │  → phase3-project-scan.sh → PROJECT_TYPE + MODULES       │  │
│  │  → phase4-detect-lark-plugin.sh → LARK_PLUGIN_INSTALLED  │  │
│  │    (检测 lark-cli 是否安装)                              │  │
│  │  → 输出：预扫描摘要                                      │  │
│  └──────────────────────────────────────────────────────────┘  │
│                               │                                 │
│              ┌────────────────┴────────────────┐                │
│              │ 模式？                          │                │
│              ├─────────────────┬───────────────┤                │
│              │ 交互式          │ 快速启动      │                │
│              │                 │               │                │
│  ┌───────────▼──────────┐  ┌──▼─────────────────────────────┐  │
│  │ 交互式确认（6步）     │  │ 参数校验                      │  │
│  │ 🚫 禁用结构化工具     │  │ → --mode + --type 必填        │  │
│  │ ⚠️ 严格单步执行       │  │ → --scope 条件必填            │  │
│  │ 步骤1: 选择分支(条件) │  │ → --branch / --upload 可选    │  │
│  │ 步骤2: 审查类型       │  │ 校验失败 → 报错终止           │  │
│  │ 步骤3: 审查范围(条件) │  │ （不降级为交互式）            │  │
│  │ 步骤4: 审查模式       │  └──┬────────────────────────────┘  │
│  │ 步骤5: 飞书上传(条件) │     │                               │
│  │ 步骤6: 执行计划确认   │     │                               │
│  └───────────┬──────────┘     │                               │
│              │                 │                               │
│              └────────┬────────┘                               │
│                       ↓                                        │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │  代码审查阶段：调用子Agent执行审查                            │  │
│  │  → 输出启动提示（含预估耗时）                           │  │
│  │  → 增量审查预处理（条件执行脚本）                       │  │
│  │  → 构造参数注入内容（参数 + 预扫描结果 + 增量数据）     │  │
│  │  → 启动子Agent                                          │  │
│  └──────────────────────────────────────────────────────────┘  │
└──────────────────────────────┬──────────────────────────────────┘
                               │ Agent调用 + 参数注入
                               ↓
┌─────────────────────────────────────────────────────────────────┐
│                  子 Agent (Review Agent)                        │
│            agents/java-code-reviewer.md 定义                    │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │  Step 1: 执行代码审查                                   │  │
│  │  → 读取 references/review-framework.md                 │  │
│  │  → 确定审查范围（根据 REVIEW_TYPE + SCOPE）            │  │
│  │  → 使用 grep/read 扫描代码                             │  │
│  │  → 按15维度检查问题                                    │  │
│  └──────────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │  Step 2: 发现归类与证据标注                             │  │
│  │  → 标注位置、证据、影响、置信度                        │  │
│  └──────────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │  Step 3: 生成审查报告                                   │  │
│  │  → 按输出格式规范生成报告（引用 report-format.md）      │  │
│  └──────────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │  Step 4: 上传飞书云文档（条件）                         │  │
│  │  → lark-cli + lark-doc skill (+create)                 │  │
│  └──────────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │  Step 5: 创建飞书多维表格（条件）                       │  │
│  │  → lark-cli + lark-base skill                          │  │
│  │    → +base-create (创建应用)                            │  │
│  │    → +table-create (18字段定义)                        │  │
│  │    → +table-delete (删除默认表)                        │  │
│  │    → +record-batch-create (批量录入)                   │  │
│  └──────────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │  Step 6: 输出最终汇总                                   │  │
│  │  → 根据FEISHU_UPLOAD_OPTION选择输出策略               │  │
│  │  → 返回结果给主Agent                                   │  │
│  └──────────────────────────────────────────────────────────┘  │
└──────────────────────────────┬──────────────────────────────────┘
                               │ 返回结果
                               ↓
┌─────────────────────────────────────────────────────────────────┐
│                         用户 (User)                             │
│                      展示审查结果                               │
└─────────────────────────────────────────────────────────────────┘
```

---

## 📊 数据流向图

```
┌──────────────┐
│ 用户提供路径  │
└──────┬───────┘
       │
       ↓
┌──────────────────────────────────────────────────────┐
│              预扫描阶段：数据采集                      │
│  (scripts/ 目录下的4个脚本顺序执行)                    │
│  → phase1: PROJECT_DIR + PROJECT_SOURCE              │
│  → phase2: IS_GIT_REPO + CURRENT_BRANCH + 分支列表   │
│  → phase3: PROJECT_TYPE + MODULE列表                  │
│  → phase4: LARK_PLUGIN_INSTALLED                     │
│                                                       │
│  输出：预扫描摘要（统一展示给用户）                   │
└──────────────────────────┬───────────────────────────┘
                           │
                           ↓
              ┌────────────────────────┐
              │  采集的环境数据变量：   │
              │  • PROJECT_DIR         │
              │  • PROJECT_SOURCE      │
              │  • PROJECT_TYPE        │
              │  • PROJECT_SCAN_RESULT │
              │  • CURRENT_BRANCH      │
              │  • LARK_PLUGIN_INSTALLED│
              └────────────┬───────────┘
                           │
              ┌────────────┴────────────┐
              │ 模式判定                │
              │ (检测 --mode 参数)      │
              ├─────────┬───────────────┤
              │交互式   │ 快速启动      │
              └────┬────┴───────┬───────┘
                   │            │
                   ↓            ↓
    ┌──────────────────┐  ┌──────────────────────┐
    │ 交互式确认（6步） │  │ 参数校验             │
    │ 🚫纯文本交互     │  │ --mode + --type 必填 │
    │ ⚠️严格单步执行   │  │ --scope 条件必填     │
    │                  │  │ 失败→终止(不降级)    │
    └────────┬─────────┘  └──────────┬───────────┘
             │                       │
             ↓                       ↓
    ┌────────────────────────────────────────┐
    │        审查配置变量：                   │
    │  • REVIEW_TYPE (增量/存量)             │
    │  • REVIEW_SCOPE (范围)                 │
    │  • REVIEW_MODE (模式)                  │
    │  • FEISHU_UPLOAD_OPTION (飞书)         │
    │  • TARGET_BRANCH (目标分支，条件)      │
    └────────────────────┬───────────────────┘
                         │
                         ↓
┌──────────────────────────────────────────────────────┐
│           代码审查阶段：准备子Agent参数注入                  │
│  ┌────────────────────────────────────────────┐     │
│  │  1. 审查任务参数表（变量注入）             │     │
│  │  2. 附加项目概况 / 增量数据                │     │
│  │  3. Agent tool 自动加载 agents/java-code-reviewer.md │     │
│  │  4. 引用路径统一使用 ${CLAUDE_PLUGIN_ROOT} │     │
│  └────────────────────────────────────────────┘     │
│                                                       │
│  增量审查时额外执行：                                 │
│  → phase5-prepare-incremental.sh                     │
│  → 输出：GIT_LOG + CHANGED_FILES + DIFF_STATS        │
└──────────────────────────┬───────────────────────────┘
                           │
                           ↓
         ┌─────────────────────────────────┐
         │  子Agent执行：                  │
         │  1. 读取审查框架                │
         │  2. 确定审查范围                │
         │  3. 执行代码扫描                │
         │  4. 生成审查报告                │
         │  5. 上传飞书（可选, lark-cli）  │
         │  6. 返回结果汇总                │
         └─────────────────────────────────┘
                           │
                           ↓
┌──────────────────────────────────────────────────────┐
│                    展示结果                           │
│  • 简化汇总 + 飞书链接（已上传飞书）                  │
│  • 完整报告（未上传飞书）                             │
└──────────────────────────────────────────────────────┘
```

---

## 💻 环境要求

### 系统要求
- **操作系统**：macOS / Linux（支持标准Bash环境）
- **Shell**：Bash 3.0+
- **Git**：用于项目扫描和分支操作

### 依赖命令
- `git` - Git操作
- `find` - 文件查找
- `grep` - 内容搜索
- `date` - 时间获取（使用 `date +%s%3N`）
- `lark-cli` - 飞书 CLI 工具（可选，用于上传云文档和多维表格）

### 无外部语言依赖
- ✅ **无Python依赖** - 所有时间戳使用 `date +%s%3N` 获取
- ✅ **纯Bash实现** - 所有脚本使用 `#!/bin/bash`
- ✅ **标准命令** - 仅使用标准命令行工具

---

## 🔧 核心组件说明

### 1. SKILL.md - 主技能定义

**作用**：定义整个技能的工作流程和规范

**核心内容**：
- 模式判定规则（交互式 / 快速启动）
- 预扫描阶段（4脚本顺序执行）
- 交互式确认规范（严格单步 + 工具禁用）
- 快速启动模式（参数规范 + 校验规则）
- 子Agent调用规范
- 10条强制规则
- 错误处理规则
- 示例对话（引用 references/examples.md）

**关键特点**：
- 脚本引用化：所有执行步骤通过 `bash ${CLAUDE_PLUGIN_ROOT}/scripts/*.sh` 调用
- 结构化交互：交互式模式使用 AskUserQuestion 分步收集配置
- 单步强制化：每个 AskUserQuestion 只负责一个步骤
- 变量标准化：统一的变量命名体系
- 双模式运行：交互式（人工）+ 快速启动（自动化）

---

### 2. agents/java-code-reviewer.md - 子Agent提示词

**作用**：定义子Agent的行为和审查标准

**核心内容**：
- 审查原则（证据驱动、高风险优先）
- 审查模式定义（模式×维度矩阵）
- Agent执行流程（6步）
- 审查框架引用（指向 references/review-framework.md）
- 输出格式规范（引用 references/report-format.md）
- 审查指南（MUST/DO/DON'T）
- 多维表格字段定义（18个字段，含3个预留修复字段）

**关键特点**：
- 引用式架构：15维度框架通过引用加载，不内联
- 参数化设计：外部参数注入，无需用户交互
- 模式化审查：严格按照模式×维度矩阵执行
- 纯Bash实现：无Python依赖，使用 `date +%s%3N` 获取时间戳

---

### 3. scripts/ - 可执行脚本目录

**作用**：提供可复用、可测试的执行脚本

**脚本列表**：

| 脚本 | 阶段 | 输入 | 输出 |
|------|------|------|------|
| phase1-detect-project.sh | 一 | 路径/Git URL | PROJECT_DIR + PROJECT_SOURCE |
| phase2-detect-branches.sh | 二A | PROJECT_DIR | 分支列表 |
| phase2-switch-branch.sh | 二C | PROJECT_DIR + 分支 + PROJECT_SOURCE | 切换结果 |
| phase3-project-scan.sh | 三 | PROJECT_DIR | PROJECT_TYPE + MODULES |
| phase4-detect-lark-plugin.sh | 四 | 无 | LARK_PLUGIN_INSTALLED (检测 lark-cli) |
| phase5-prepare-incremental.sh | 五 | PROJECT_DIR + N | GIT_LOG + FILES + STATS |

**关键特点**：
- 独立可测试：每个脚本可单独运行验证
- 输出标准化：KEY=value 格式便于解析
- 错误处理：set -e 确保错误及时退出
- 纯Bash实现：无Python依赖，兼容Linux标准环境

---

### 4. references/review-framework.md - 审查框架手册

**作用**：定义15个维度的审查标准和模式×维度矩阵

**核心内容**：
- 15个审查维度的详细定义
- 模式×维度覆盖矩阵
- 各模式的启用范围说明
- 版本信息

**关键特点**：
- 独立维护：修改框架无需更新Agent提示词
- 版本同步：与Agent提示词版本号保持一致
- 模式化定义：清晰的启用/跳过规则

---

### 5. 多维表格字段体系

**作用**：定义飞书多维表格的问题追踪字段结构

**字段分类（共18个字段）**：

| 类别 | 字段数 | 字段列表 |
|------|--------|----------|
| 标识类 | 3 | 问题编号、严重级别、所属维度 |
| 描述类 | 5 | 问题描述、位置、证据、影响、修复建议 |
| 管理类 | 7 | 置信度、修复状态、审查模式、审查日期、负责人、备注、**修复时间** |
| 协作类 | 3 | 技术栈、**修复分支**、**修复人** |

**预留修复字段（v5.1新增）**：
- **修复时间**：记录实际修复完成时间
- **修复分支**：记录修复所在的分支名（如 fix/issue-123）
- **修复人**：记录实际修复人员

这些字段初始留空，配合"修复状态"字段形成完整的修复工作流追踪链路。

---

## 🎯 运行框架特点

### 1. 双Agent架构

```
主Agent (协调者)          子Agent (执行者)
    ↓                          ↓
• 模式判定                  • 代码审查
• 预扫描编排                • 问题发现
• 交互式确认(纯文本)        • 报告生成
• 快速启动参数校验          • 飞书上传(lark-cli)
• 参数准备与注入            • 结果汇总
• 结果展示
```

**优势**：
- 主Agent专注用户交互和流程控制
- 子Agent专注代码审查任务
- 长时间审查不阻塞主Agent
- 用户可在审查期间继续使用 Claude Code

### 2. 双模式运行

| 维度 | 交互式模式 | 快速启动模式 |
|------|-----------|-------------|
| 触发 | 不含 `--mode` | 包含 `--mode` 参数 |
| 适用场景 | 人工审查 | 定时任务/CI/CD |
| 用户交互 | 逐步确认（6步） | 无（跳过交互） |
| 参数来源 | 文本选项交互 | `--` 参数直接传入 |
| 参数校验失败 | 提示重试 | 直接报错终止（不降级） |
| 执行计划确认 | 必须 | 跳过 |

### 3. 严格单步交互规则

交互式模式下，主Agent必须遵守以下约束：

| 规则 | 说明 |
|------|------|
| 使用结构化交互 | 必须使用 `AskUserQuestion` 收集用户选择 |
| 单步执行 | 每次只收集一个步骤的配置 |
| 禁止合并 | 不得将多个步骤（如审查范围+审查模式）放在同一次交互中 |
| 条件跳过 | 仅允许按技能规则自动跳过条件步骤（如单模块存量审查的范围选择） |
| 快速启动豁免 | 检测到 `--mode` 后跳过交互式确认，直接执行参数校验与审查 |

### 4. 脚本化工作流

**优势**：
- 脚本可独立维护和测试
- 修改脚本无需更新文档
- 输出格式标准化便于解析
- 可复用于其他工具

### 5. 引用式架构

**优势**：
- 审查框架独立维护（review-framework.md）
- 报告格式独立维护（report-format.md）
- Agent提示词精简（格式和框架按需加载，核心逻辑保持精简）
- 版本同步机制清晰
- 修改格式/框架不影响提示词结构

### 6. 变量标准化

**命名规则**：
```bash
# 项目相关
PROJECT_DIR           # 项目根目录
PROJECT_NAME          # 项目名称
PROJECT_TYPE          # Maven/Gradle + 单/多模块
PROJECT_SCAN_RESULT   # 项目扫描结果

# Git相关
CURRENT_BRANCH        # 当前分支
TARGET_BRANCH         # 目标分支
GIT_LOG_OUTPUT        # Git提交记录
CHANGED_FILES_OUTPUT  # 变更文件列表
DIFF_STATS_OUTPUT     # 变更统计

# 审查配置
REVIEW_TYPE           # 审查类型
REVIEW_SCOPE          # 审查范围
REVIEW_MODE           # 审查模式
FEISHU_UPLOAD_OPTION  # 飞书上传选项
```

---

## 📈 优化历程

| 版本 | 优化内容 | 效果 |
|------|---------|------|
| 原版 | 所有内容内嵌在文档中 | 难以维护 |
| 5.0-alpha | 脚本抽取到scripts/目录 | 脚本可独立测试 |
| 5.0-beta | 变量命名优化 | 更清晰的变量语义 |
| 5.0 | 审查框架抽取到references/ | Agent提示词精简23% |
| 5.1 | 多维表格字段扩展（15→18） | 新增3个修复预留字段 |
| 5.1 | 移除Python依赖 | 纯Bash实现，兼容标准环境 |
| 5.2 | 预扫描阶段重构 | 4脚本合并为统一的预扫描阶段 |
| 5.2 | 新增快速启动模式 | 支持定时任务/CI/CD自动化场景 |
| 5.2 | 强化交互式确认规则 | 工具禁用 + 严格单步执行 + 反例 |
| 5.3 | 飞书上传迁移至 lark-cli | 移除旧版 feishu_ 工具依赖，统一使用 lark-cli + lark-doc/lark-base skills |
| 5.4 | 全面结构优化 | 输出格式外置 + 示例外置 + 矩阵去重 + 工具名修正 + 脚本重编号，核心文件减重30% |
| 5.5 | 扫描策略重构 | 子代理从「按维度遍历」重构为「逐文件单次读取多维度同时评估」，审查效率提升 |
| 5.5 | bitable-schema 内联 | 多维表格字段定义统一内联到子代理，减少额外依赖和漂移风险 |

---

## 🔗 文件依赖关系

```
SKILL.md
  ├── 模式判定 → 交互式 / 快速启动
  ├── 预扫描阶段 → scripts/phase1~4.sh (4个脚本)
  ├── 交互式确认 → AskUserQuestion 分步收集配置
  ├── 代码审查阶段 → agents/java-code-reviewer.md
  └── 示例对话 → references/examples.md

agents/java-code-reviewer.md
  ├── 引用 → references/review-framework.md（审查框架 + 维度矩阵）
  ├── 引用 → references/report-format.md（报告输出格式）
  ├── 内联 → 多维表格字段定义（执行时唯一真相源）
  ├── 注入 → 外部参数（由主Agent提供）
  ├── 飞书云文档上传 → lark-cli + lark-doc skill
  └── 多维表格操作 → lark-cli + lark-base skill

scripts/*.sh
  └── 独立执行 → 系统命令 (git, find, date等; lark-cli 仅检测安装状态)

references/
  ├── review-framework.md   → 审查标准定义 + 模式×维度矩阵
  ├── report-format.md      → 报告输出格式规范
  └── examples.md           → 示例对话（5个场景）
```

### 多维表格字段结构（18个字段）

**基础字段（15个）**：
1. 问题编号、2. 严重级别、3. 所属维度、4. 技术栈、5. 问题描述
6. 位置、7. 置信度、8. 证据、9. 影响、10. 修复建议
11. 修复状态、12. 审查模式、13. 审查日期、14. 负责人、15. 备注

**预留修复字段（3个）**：
16. **修复时间** - 日期类型，初始留空
17. **修复分支** - 文本类型，初始留空
18. **修复人** - 人员类型，初始留空

---

## 🚀 执行流程总结

### 交互式模式

1. **用户触发** → 主Agent接收请求
2. **预扫描** → 4脚本顺序执行采集项目数据 → 输出预扫描摘要
3. **交互式确认** → 6步纯文本选项交互（严格单步）
4. **代码审查阶段** → 构造prompt并调用子Agent
5. **子Agent执行** → 读取审查框架 + 扫描代码 + 生成报告
6. **飞书上传** → 条件执行（根据用户选择）
7. **结果展示** → 主Agent展示结果给用户

### 快速启动模式

1. **用户触发**（含 `--mode` 参数） → 主Agent接收请求
2. **预扫描** → 4脚本顺序执行采集项目数据
3. **参数校验** → 校验必填参数，失败则终止（不降级为交互式）
4. **代码审查阶段** → 直接构造prompt并调用子Agent（跳过交互确认）
5. **子Agent执行** → 同上
6. **结果展示** → 主Agent展示结果给用户

---

*架构文档版本：5.4*
*最后更新：2026-04-27*

## 🆕 版本 5.5 更新说明

### 子代理扫描策略重构
将子代理的代码扫描策略从「按维度遍历」重构为「逐文件单次读取，多维度同时评估」：
- **旧策略**：对每个维度分别扫描所有文件，同一文件被多次读取
- **新策略**：按风险优先级排序文件，每个文件只读一次，读完立即在当前上下文中完成所有启用维度的评估
- 引入阶段 A（收集路径）→ 阶段 B（风险排序）→ 阶段 C（逐文件评估）→ 阶段 D（定向补充）的四阶段流程

### bitable-schema 内联执行
多维表格字段定义统一保留在 `agents/java-code-reviewer.md` 中，作为建表时的唯一真相源。

这样可以避免维护两份字段定义带来的漂移风险，也不需要在运行时额外读取参考文件。

### 修复优先级排序优化
修复审查报告中问题按维度分组输出的问题，改为严格按 P0 → P1 → P2 → P3 → 待确认 优先级排序。

### 聚合位置格式修正
统一问题位置标注格式，多处出现的同类问题只列一个代表性位置并标注总数（如 `A.java:45（同类问题共3处）`）。

---

## 🆕 版本 5.4 更新说明

### 全面结构优化
- **输出格式外置**：报告格式规范抽取到 `references/report-format.md`
- **示例对话外置**：示例对话抽取到 `references/examples.md`
- **矩阵去重**：模式×维度覆盖矩阵统一维护在 `references/review-framework.md`，去除 prompt 中的重复定义
- **脚本重编号**：脚本按预扫描阶段顺序统一编号为 phase1~phase5
- 核心提示词文件减重约 30%

---

## 🆕 版本 5.3 更新说明

### 飞书上传迁移至 lark-cli

将飞书集成从旧版 `feishu_*` MCP 工具迁移至 `lark-cli` 命令行工具及配套 skills：

**检测阶段变更**：
- 移除对 openclaw-feishu / openclaw-wkzj 插件的检测
- `phase4-detect-lark-plugin.sh` 改为仅检测 `lark-cli` 是否安装（`command -v lark-cli`）
- 移除对 `jq` 的依赖（不再解析 openclaw.json）

**上传工具变更**：
- 上传飞书云文档：从 `feishu_create_doc` 迁移至 `lark-cli` + `lark-doc` skill（`+create`）
- 上传多维表格：从 `feishu_bitable_*` 系列工具迁移至 `lark-cli` + `lark-base` skill（`+base-create`、`+table-create`、`+table-delete`、`+record-batch-create`）

**lark-cli 安装**：
```bash
npm install -g @larksuite/cli
npx skills add larksuite/cli -y -g
```

详细安装指南见 [lark-cli README](https://github.com/larksuite/cli/blob/main/README.zh.md)。

---

## 🆕 版本 5.2 更新说明

### 预扫描阶段重构
将原来的阶段一至阶段四合并为统一的「预扫描阶段」，4个脚本按顺序自动执行，不与用户交互，全部完成后统一输出预扫描摘要。

### 快速启动模式
新增快速启动模式，支持定时任务、自动化脚本、CI/CD 集成等无需人工交互的场景：
- 通过 `--mode` 参数触发，跳过所有交互式确认
- 完整的参数规范（`--mode`、`--type`、`--scope`、`--branch`、`--upload`）
- 严格的参数校验，缺失必填参数直接报错终止，不降级为交互式模式

### 强化交互式确认规则
针对模型合并多步骤选项的问题，新增以下规则：
- **使用 AskUserQuestion**：交互式模式必须使用结构化单选交互收集配置
- **严格单步执行**：每次交互只能处理一个步骤
- **典型错误反例**：明确展示"审查范围+审查模式"合并展示的错误场景
- **步骤边界约束**：每个步骤结尾明确标注"不得同时输出下一步骤的选项"

### 多维表格字段扩展（15 → 18）（v5.1）
新增 3 个预留修复字段，用于完整的问题修复追踪：
- **修复时间**（日期）：记录实际修复完成时间
- **修复分支**（文本）：记录修复所在的分支名（如 fix/issue-123）
- **修复人**（人员）：记录实际修复人员

这些字段初始留空，供后续修复流程更新使用。

### 移除 Python 依赖（v5.1）
- 将时间戳获取从 `python3 -c "import datetime..."` 替换为纯 Bash 命令 `date +%s%3N`
- 所有脚本保持纯 Bash 实现（`#!/bin/bash`）
- 确保在仅支持 Bash/Shell 的标准环境中正常运行
